...
void btn_callback(uint gpio, uint32_t events) {
    if (events == 0x4) {
        if (gpio == BTN_PIN_R) {
            ...
            int val = 1;
            xQueueSendFromISR(xQueueBtnG_irq, &val, 0);
        }
    }
}
...
void btn_2_task(void *p) {
    ...
    int dummy;
    while (true) {
        if (xQueueReceive(xQueueBtnG_irq, &dummy, pdMS_TO_TICKS(500)) == pdTRUE) {
            if (delay < 1000) {
                delay += 100;
            } else {
                delay = 100;
...
int main() {
    ...
    xQueueBtnG_irq = xQueueCreate(32, sizeof(int));
